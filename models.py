...
class Player(db.Model):
    ...
    def nickname_gradient(self):
        """Get nickname gradient CSS"""
        gradient_setting = PlayerGradientSetting.query.filter_by(
            player_id=self.id,
            element_type='nickname',
            is_enabled=True
        ).first()
        
        if gradient_setting and gradient_setting.gradient_theme:
            return gradient_setting.gradient_theme.css_gradient
        return None
    ...
    def display_role(self):
        """Get the role to display (admin role > custom role > regular role)"""
        # Check for active admin role first
        if hasattr(self, 'active_admin_role') and self.active_admin_role:
            return self.active_admin_role.name
        # Then check for custom role
        elif hasattr(self, 'custom_role_purchased') and self.custom_role_purchased and self.custom_role:
            return self.custom_role
        # Finally fallback to regular role
        return self.role
