...
# This route is being replaced by the new /shop/purchase API endpoint
...
@app.route('/admin/assign_role', methods=['POST'])
def admin_assign_role():
    """Assign admin role to player with real-time updates"""
    if not session.get('is_admin', False):
        flash('Доступ запрещен!', 'error')
        return redirect(url_for('login'))
    ...
    try:
        ...
        if not player_id or not role_id:
            flash('Необходимо выбрать игрока и роль!', 'error')
            return redirect(url_for('admin_roles'))
        ...
        existing_roles = PlayerAdminRole.query.filter_by(player_id=player_id).all()
        for existing_role in existing_roles:
            existing_role.is_active = False

        # Assign new role
        existing_assignment = PlayerAdminRole.query.filter_by(
            player_id=player_id, 
            role_id=role_id
        ).first()
        ...
        if existing_assignment:
            ...
            new_assignment = PlayerAdminRole(
                player_id=player_id,
                role_id=role_id,
                is_active=True,
                assigned_at=datetime.utcnow()
        ...
        flash(f'Роль "{role.name}" успешно назначена игроку {player.nickname}!', 'success')
...
@app.route('/admin/assign_gradient', methods=['POST'])
def admin_assign_gradient():
    """Assign gradient to player with real-time updates"""
    if not session.get('is_admin', False):
        flash('Доступ запрещен!', 'error')
        return redirect(url_for('login'))
    ...
    try:
        player_id = request.form.get('player_id', type=int)
        gradient_id = request.form.get('gradient_id', type=int)
        element_type = request.form.get('element_type', 'nickname')

        if not player_id or not gradient_id:
            flash('Необходимо выбрать игрока и градиент!', 'error')
            return redirect(url_for('admin_gradients'))

        player = Player.query.get_or_404(player_id)
        gradient = GradientTheme.query.get_or_404(gradient_id)

        # Remove existing gradient for this element type
        existing = PlayerGradientSetting.query.filter_by(
            player_id=player_id,
            element_type=element_type
        ).first()

        if existing:
            existing.gradient_theme_id = gradient_id
            existing.is_enabled = True
            existing.assigned_at = datetime.utcnow()
        else:
            new_setting = PlayerGradientSetting(
                player_id=player_id,
        ...
        flash(f'Градиент "{gradient.name}" назначен для {element_type} игрока {player.nickname}!', 'success')
        ...
        app.logger.error(f"Error assigning gradient: {e}")
        flash('Ошибка при назначении градиента!', 'error')
        return redirect(url_for('admin_gradients'))
@app.route('/admin/gradients')
def admin_gradients():
    """Admin gradient management"""
    ...
    try:
        gradients = GradientTheme.query.filter_by(is_active=True).all()
        players = Player.query.order_by(Player.nickname).all()
        
        # Get current assignments
        assignments = {}
        for player in players:
            player_assignments = PlayerGradientSetting.query.filter_by(
                player_id=player.id,
                is_enabled=True
            ).all()
            assignments[player.id] = {setting.element_type: setting for setting in player_assignments}
        ...
        app.logger.error(f"Error in admin gradients: {e}")
        flash('Ошибка при загрузке градиентов!', 'error')
        return redirect(url_for('admin'))
@app.route('/admin/clear_gradients/<int:player_id>', methods=['POST'])
def admin_clear_gradients(player_id):
    """Clear all gradients for a player"""
    if not session.get('is_admin', False):
        flash('Доступ запрещен!', 'error')
        return redirect(url_for('login'))
    ...
    try:
        ...
        # Disable all gradient settings for this player
        PlayerGradientSetting.query.filter_by(player_id=player_id).update({'is_enabled': False})
        
        db.session.commit()
        
        flash(f'Все градиенты очищены для игрока {player.nickname}!', 'success')
        
        # Clear cache
        Player.clear_statistics_cache()
        
    except Exception as e:
        app.logger.error(f"Error clearing gradients: {e}")
        flash('Ошибка при очистке градиентов!', 'error')
