...
// Modern Leaderboard Module
function initializeModernLeaderboard() {
    createModernLeaderboardContainer();
    loadLeaderboardData();
    setupRealtimeUpdates();
}

function createModernLeaderboardContainer() {
    const oldTable = document.querySelector('.leaderboard-table');
    if (!oldTable) return;

    const container = document.createElement('div');
    container.id = 'modern-leaderboard';
    container.className = 'modern-leaderboard-container';
    container.innerHTML = `
        <div class="leaderboard-header">
            <h3>Современный лидерборд</h3>
            <div class="leaderboard-controls">
                <select id="sortSelector" class="form-select">
                    ...
                    <option value="final_kills">По финальным убийствам</option>
                    <option value="beds_broken">По кроватям</option>
                    <option value="wins">По победам</option>
                </select>
                <button id="refreshBoard" class="btn btn-primary">
    ...
    document.getElementById('sortSelector').addEventListener('change', loadLeaderboardData);
    document.getElementById('refreshBoard').addEventListener('click', loadLeaderboardData);
}

async function loadLeaderboardData() {
    const grid = document.getElementById('leaderboardGrid');
    const sortBy = document.getElementById('sortSelector')?.value || 'experience';
    
    if (!grid) return;

    grid.innerHTML = '<div class="loading-spinner"><i class="fas fa-spinner fa-spin"></i> Загрузка...</div>';

    try {
        const response = await fetch(`/api/leaderboard?sort=${sortBy}&format=modern`);
        const data = await response.json();
        
        if (data.success) {
            renderModernLeaderboard(data.players);
        } else {
            grid.innerHTML = '<div class="error-message">Ошибка загрузки данных</div>';
        }
    } catch (error) {
        console.error('Error loading leaderboard:', error);
        grid.innerHTML = '<div class="error-message">Ошибка подключения</div>';
    }
}

function renderModernLeaderboard(players) {
    const grid = document.getElementById('leaderboardGrid');
    if (!grid) return;
    ...
    players.forEach((player, index) => {
        ...
        const roleHtml = renderPlayerRole(player);
        const nicknameHtml = renderPlayerNickname(player);
        
        playerCard.innerHTML = `
            <div class="player-rank">#${index + 1}</div>
            <div class="player-avatar">
                <img src="${player.skin_url || `/static/img/skins/steve.png`}" 
                     alt="${player.nickname}" 
                     onerror="this.src='/static/img/skins/steve.png'">
            </div>
            <div class="player-info">
                <div class="player-nickname">${nicknameHtml}</div>
                <div class="player-role">${roleHtml}</div>
            </div>
            <div class="player-stats">
                ...
                <div class="stat-group">
                    ...
                    <span class="stat-value">${player.experience.toLocaleString()}</span>
                </div>
                <div class="stat-group">
...
function renderPlayerRole(player) {
    if (player.admin_role) {
        const role = player.admin_role;
        let style = '';
        
        if (role.has_gradient && role.gradient_start && role.gradient_end) {
            style = `background: linear-gradient(45deg, ${role.gradient_start}, ${role.gradient_end}); 
                     background-size: 200% 200%; 
                     -webkit-background-clip: text; 
                     -webkit-text-fill-color: transparent; 
                     background-clip: text;`;
            
            if (role.gradient_animated) {
                style += ' animation: gradientShift 3s ease-in-out infinite;';
            }
        } else if (role.color) {
            style = `color: ${role.color};`;
        }

        const emoji = role.emoji_url ? 
            `<img src="${role.emoji_url}" class="role-emoji" alt="emoji">` : 
            (role.emoji_class ? `<i class="${role.emoji_class}"></i>` : 
            (role.emoji ? role.emoji : ''));

        return `<span class="role-admin" style="${style}">${emoji} ${role.name}</span>`;
    } else if (player.custom_role_purchased && player.custom_role) {
        let style = '';
        
        if (player.custom_role_gradient) {
            style = `background: ${player.custom_role_gradient}; 
                     background-size: 200% 200%; 
                     -webkit-background-clip: text; 
                     -webkit-text-fill-color: transparent; 
                     background-clip: text;`;
            
            if (player.custom_role_animated) {
                style += ' animation: gradientShift 3s ease-in-out infinite;';
            }
        } else if (player.custom_role_color) {
            style = `color: ${player.custom_role_color};`;
        }

        const emoji = player.custom_role_emoji ? player.custom_role_emoji : '';
        return `<span class="role-custom" style="${style}">${emoji} ${player.custom_role}</span>`;
    } else {
        return `<span class="role-default">${player.role || 'Игрок'}</span>`;
    }
}

function renderPlayerNickname(player) {
    if (player.nickname_gradient) {
        return `<span class="nickname-gradient" style="background: ${player.nickname_gradient}; 
                background-size: 200% 200%; 
                -webkit-background-clip: text; 
                -webkit-text-fill-color: transparent; 
                background-clip: text;
                animation: gradientShift 3s ease-in-out infinite;">${player.nickname}</span>`;
    } else {
        return `<span class="nickname-default">${player.nickname}</span>`;
    }
}
...
function setupRealtimeUpdates() {
    ...
    // Listen for role/gradient updates
    document.addEventListener('roleUpdated', () => {
        loadLeaderboardData();
    });
    
    document.addEventListener('gradientUpdated', () => {
        loadLeaderboardData();
    });
}
...
function toggleLeaderboardView(view) {
    const classicTable = document.querySelector('.table-responsive');
    const modernBoard = document.getElementById('modern-leaderboard');
    
    if (view === 'modern') {
        if (classicTable) classicTable.style.display = 'none';
        if (modernBoard) modernBoard.style.display = 'block';
        localStorage.setItem('leaderboardView', 'modern');
    } else {
        if (classicTable) classicTable.style.display = 'block';
        if (modernBoard) modernBoard.style.display = 'none';
        localStorage.setItem('leaderboardView', 'classic');
    }
}

// Initialize saved view preference
document.addEventListener('DOMContentLoaded', function() {
    const savedView = localStorage.getItem('leaderboardView') || 'classic';
    const classicRadio = document.getElementById('classic-view');
    const modernRadio = document.getElementById('modern-view');
    
    if (savedView === 'modern' && modernRadio) {
        modernRadio.checked = true;
        toggleLeaderboardView('modern');
    } else if (classicRadio) {
        classicRadio.checked = true;
        toggleLeaderboardView('classic');
    }
});